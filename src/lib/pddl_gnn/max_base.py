...
class Readout(nn.Module):
    ...
    def forward(self, batch_num_objects: List[int], node_states: Tensor) -> Tensor:
        # cumsum_indices = torch.tensor(batch_num_objects, device=node_states.device).cumsum(0) - 1
        cumsum_indices = batch_num_objects.clone().detach().cumsum(0) - 1
...
class RelationMessagePassingModel(nn.Module):
    def __init__(
      self, relations: list, hidden_size: int, iterations: int,
      init_only_zeros=False):
        super().__init__()
        self.hidden_size = hidden_size
        self.iterations = iterations
        self.relation_network = RelationMessagePassing(relations, hidden_size)
        self.dummy = nn.Parameter(torch.empty(0))
        self.init_only_zeros = init_only_zeros
    ...
    def _initialize_nodes(self, num_objects: int) -> Tensor:
        ...
        if self.init_only_zeros:
            init_nodes = torch.cat([init_zeroes, init_zeroes], dim=1)
        else:
...
class MaxModelBase(pl.LightningModule):
    def __init__(self, predicates: list, hidden_size: int, iterations: int, 
                 output_graph_state=False, init_only_zeros=False):
        super().__init__()
        self.output_graph_state = output_graph_state
        self.save_hyperparameters()
        self.model = RelationMessagePassingModel(
          predicates, hidden_size, iterations, init_only_zeros=init_only_zeros
        )
    ...
    def forward(self, states: Tuple[Dict[int, Tensor], List[int]]) -> Tensor:
        ...
        if self.output_graph_state:
            return torch.abs(self.readout(states[1], node_states))
        return node_states
